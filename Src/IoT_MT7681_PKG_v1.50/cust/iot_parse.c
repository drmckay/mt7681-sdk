...
#if (XWIFI_BOARD_SUPPORT==1)
#define HUM_TEMP_GPIO 0
UINT_16 g_hum_data=0;
UINT_16 g_temp_data=0;
#endif
						...
						IoTPacketInfo  *pPacketInfo)
{
...
#if (XWIFI_BOARD_SUPPORT==1) 
		HUM_TEMP_Information *HUM_TEMP_out;//Temperature-Humidity sensor 
#endif
...
#if (XWIFI_BOARD_SUPPORT==1) 
		HUM_TEMP_out=(HUM_TEMP_Information*)Data_out; 
#endif
		...
		switch (FunctionType)
		{
			...
			case UART_GET_REQUEST:
				...
				break;
#if (XWIFI_BOARD_SUPPORT==1)
			case HUM_TEMP_GET_REQUEST:	//Temperature-Humidity sensor 			
				DBGPRINT/*DBGPRINT_HIGH*/(RT_DEBUG_TRACE,("HUM_TEMP_GET_REQUEST\n"));
				
				IoT_build_app_response_header(iot_buffer, FUNCTION, HUM_TEMP_GET_RESPONSE, HUM_TEMP_INFORMATION, sizeof(HUM_TEMP_Information), pPacketInfo);

				if(!DHT11_get_data())
				{
					HUM_TEMP_out->HUM_Value=g_hum_data;
					HUM_TEMP_out->TEMP_Value=g_temp_data;
				}
				
				DBGPRINT/*DBGPRINT_HIGH*/(RT_DEBUG_TRACE,("humidity=0x%x,temperature=0x%x\n",HUM_TEMP_out->HUM_Value,HUM_TEMP_out->TEMP_Value ));				
				payload_len = (UINT16)(CP_HDR_LEN+CP_DATA_HDR_LEN+sizeof(HUM_TEMP_Information));					
				break;			
#endif										
		}
...
#if (XWIFI_BOARD_SUPPORT==1)
//************DHT11 is Humidity and Temperature sensor on xWifi board*******//
//*******************DHT11 sensor driver ,xWifi board********************//
UINT8 DHT11_start(void)
{
	UINT8 start_flag,input,Polarity; 
	UINT32 gpio_input;

	 start_flag = 0;

	//Printf_High("Enter into DHT11_start\n");

#if ((IOT_PWM_SUPPORT == 1) && (IOT_PWM_TYPE == 2))
	/*if this GPIO has been set as PWM, cancel PWM first*/
	IoT_software_pwm_del(HUM_TEMP_GPIO);
#endif

 	IoT_gpio_output((INT32)HUM_TEMP_GPIO,1);
 	usecDelay(1);//	 delay_us(1);
	IoT_gpio_output((INT32)HUM_TEMP_GPIO,0);
 	usecDelay(8200);////	 delay_ms(20);	//the dealy should be larger than 18ms
  	IoT_gpio_output((INT32)HUM_TEMP_GPIO,1);
 	usecDelay(7);////	 delay_us(40);     //the dealy should be around 20~40us
	IoT_gpio_input((INT32)HUM_TEMP_GPIO, &gpio_input);	
	while(!gpio_input)
	{	start_flag = 1;
	IoT_gpio_input((INT32)HUM_TEMP_GPIO, &gpio_input);	
	}	
...
UINT8 DHT11_get_data(void)
{
	UINT8 check=0,sum=0;

	if(DHT11_start())
	{
		check=DHT11_get_byter();		
		sum +=check;
		g_hum_data=check;
		g_hum_data <<=8;
	...
	//Printf_High("check=0x%x,sum=0x%x\n",check,sum);
